...
BOOL_TOOLTIPS = {
    "allow_gas_release": "Allow for gaseous reaction products, e.g. O2, CO2",
...
def layout_func(app):
    layout = html.Div(
        [
            ...
            html.Button("run", id="run_button", style=button_style),
            html.Details(
                [
                    html.Summary("Advanced options"),
                    html.Div(
                        [
                            html.Span("Temperature", style=label_style),
                            dcc.Input(
                                id="temperature",
                                placeholder="1600 K",
                                style=input_style,
                    ...
                    html.Div(
                        [
                            html.Span("Pressure", style=label_style),
                            dcc.Input(
                                id="pressure",
                                placeholder="0.001 atm",
                                style=input_style,
                            ),
                        ],
                        style=input_container_style,
                    ...
                    html.Div(
                        [
                            html.Span("Add element", style=label_style),
                            dcc.Input(
                                id="add_element", placeholder="None", style=input_style
                            ),
                        ],
                        style=input_container_style,
                    ),
                    html.Div(
                        [
                            html.Span("Max precursors", style=label_style),
                            dcc.Input(
                                id="max_component_precursors",
                                placeholder="2",
                                style=input_style,
                            ),
                        ],
                    ...
                    dcc.Checklist(
                        ...
                        options=[
                            {"label": BOOL_TOOLTIPS[bo], "value": bo}
                ...
                style=advanced_container_style,
            ),
            dcc.Loading(
                id="loading-1", children=[html.Div(id="synthesis_output")], type="cube"
            ),
        ],
        style=layout_style,
    )
    ...
    @app.callback(
        Output("synthesis_output", "children"),
    ...
    def update_results(
        ...
        try:
            # Marshall options
            temperature = float(temperature) if temperature else 1600.0
            pressure = float(pressure) if pressure else None
            max_component_precursors = (
                int(max_component_precursors) if max_component_precursors else 2
            )
            add_element = add_element
            synthesis_bool_options = {
                bo: True if bo in synthesis_bool_options else False
                for bo in BOOL_OPTIONS
            }
            ...
            if not value.startswith("mp"):
                with MPRester() as mpr:
                    ...
                    options = mpr.query(
                        {"pretty_formula": formula}, ["material_id", "e_above_hull"]
                    )
                    if not options:
                        raise ValueError(
                            "{} query failed, please enter valid formula or mp id".format(
                                value
                            )
                        )
                    options = sorted(options, key=lambda x: x["e_above_hull"])
                    value = options[0]["material_id"]
            router = SynthesisRoutes(value, add_elements=[add_element], use_cache_database=False)
            ...
            return html.Div(children=[str(e)], style={"color": "red"})
...
def generate_table(dataframe):
    return html.Table(
        ...
        # Body
        [
            html.Tr([html.Td(str(dataframe.iloc[i][col])) for col in dataframe.columns])
...
def serialize_matpipe(pipe):
    stringio = StringIO()
    for obj in [pipe, pipe.learner, pipe.reducer, pipe.cleaner, pipe.autofeaturizer]:
        obj._logger = None
